...
console.log(0O11110111); //2396233
```
> 如果要将`0b`和`0o`前缀的字符串数字转为纸巾纸,要使用`Number`方法
```javascript
...
```
## 5.5 安全整数和`Number.isSafeInteger()`
...
+ 5.6.1 `Math.trunc()`用于去除一个数的小数部分,返回整数部分
```javascript
...
```
+ 5.6.2 `Math.sign()`用来判断一个数到底是正数,负数还是零
```javascript
...
```
+ 5.6.4 `Math.clz32()`返回一个数的 32 位无符号整数形式有多少个前导 0
```javascript
...
```
> `clz32`这个函数名就来自`count leading zero bits in 32-bit binary representation of a number`计算一个数的32位二进制形式的前导0的个数的缩写
...
```
+ 5.6.5 `Math.imul()`返回两个数以32位带符号整数形式相乘的结果,返回的也是一个 32 位的带符号整数
```javascript
...
+ 5.6.6 `Math.fround()`返回一个数的单精度浮点数形式
```javascript
...
+ 5.6.7 `Math.hypot()`返回所有参数的平方和的平方根
```javascript
...
```
+ `Math.log10(x)`方法返回以`10 为底的x的对数`.如果x小于0,则返回`NaN`
```javascript
...
+ `Math.log2(x)`方法返回以`2 为底的x的对数`.如果x小于0,则返回`NaN`
```javascript
